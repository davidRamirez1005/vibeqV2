    ...
    <ul class="main-menu">
        ...
        :class="`${mainmenuItem.menuTitle ? 'slide__category' : ''} ${mainmenuItem?.type == 'link'? 'slide' : ''} ${mainmenuItem?.type == 'empty'? 'slide' : ''} ${mainmenuItem?.type == 'sub' ? 'slide has-sub' : ''} ${mainmenuItem?.active ? 'open' : ''} ${mainmenuItem?.selected ? 'active' : ''}`">
            ...
            <template v-if="mainmenuItem?.type === 'link'">
                <a href="javascript:;"  @click="handleLinkClick($event, mainmenuItem.path); setSubmenu(mainmenuItem.path)" class="side-menu__item" :class="`${mainmenuItem.selected ? 'active' : ''}`" >
...
<script>
import landingrecursiveMenu from './landingrecursiveMenu.vue';
export default {
    props: {
        menuData: Object
    },
